...
df = pd.read_excel(r"C:\Users\adidu\Documents\Work stuff\Year 3\Computing Project\Stuff 2.xlsx")
...
def BigFunc(T, t):
    ...
    for i in range (0,int(T-1)):
        ...
        #Jradius[i] = (xs[2] - A) - np.sqrt((Xmatrix[i,2]- A)**2 + (Ymatrix[i,2]- B)**2)
        for j in range (0,N):
    ...
    return VXmatrix, VYmatrix, Xmatrix, Ymatrix, Eradius, Jradius
...
def dtvary():
    ...
    for i in range(0,Y):
        ...
        T = 365 * 60 * 60 * 24 * 20
        TotT = T / dt
        Vx, Vy, X, Y, R1, R2 = BigFunc(TotT, dt)
...
#Vx, Vy, X, Y, R1, R2 = BigFunc(oTotT, odt)
