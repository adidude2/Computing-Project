...
def plotfig(R, SavedCOM, SavedX, SavedY, SavedZ, SavedSteps, t, view, filename):
    ...
    ani.save(r"C:\Users\adidu\Documents\Work stuff\Year 3\Computing Project\Energy diagnostic plots\Animations\\"
             + filename, fps=30)
...
def EnergyPlotfrac(t, R, Mhalo, Rvir, c, SavedCOM, SavedX, SavedY, SavedZ, Vx, Vy, Vz, mass, SavedSteps, Rs, rho0):
    ...
    Toomreheatmap(SavedX, SavedY, Vx, Vy, SavedSteps, R, mass, "Toomre Heatmap (2000p).mp4", t)
    ...
    #axes[2].legend()
    fig.savefig(r'C:\Users\adidu\Documents\Work stuff\Year 3\Computing Project\Energy diagnostic plots\Animations\Energy Analysis.png', transparent=True)
...
def Toomreheatmap(X, Y, Vx, Vy, SavedSteps, Rmax, mass, filename, t):
    ...
    pcm = ax.pcolormesh([[0,0],[0,0]], cmap='inferno', vmin=0, vmax=3)
    cbar = fig.colorbar(pcm, ax=ax, label='Toomre Q')
    ...
    def update(frame):
        ...
        kappa2 = 4*Omega2 + R_centers*dOmega2_dR
        kappa2 = np.where(kappa2 > 0, kappa2, 0)
